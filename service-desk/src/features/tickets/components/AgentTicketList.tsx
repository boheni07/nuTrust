  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-slate-900">티켓 목록</h1>
      </div>
        ...
        <table className="w-full text-sm">
          <thead className="bg-slate-50 border-b border-slate-200">
            <tr>
              ...
              <th className="px-4 py-3 text-left font-medium text-slate-600">요청자</th>
              ...
              tickets.map((ticket: any) => (
                <tr key={ticket.id} className="hover:bg-slate-50 transition-colors">
                  <td className="px-4 py-3">
                    <Link href={`/agent-tickets/${ticket.id}`} className="font-mono text-blue-600 hover:underline">{ticket.ticketNumber}</Link>
                  </td>
                  <td className="px-4 py-3 font-medium text-slate-900 max-w-xs truncate">
                    <Link href={`/agent-tickets/${ticket.id}`} className="hover:text-blue-600">{ticket.title}</Link>
                  </td>
