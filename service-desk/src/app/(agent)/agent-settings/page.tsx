...
export default function AgentSettingsPage() {
  ...
  const updateProfile = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['my-profile'] }); setEditMode(false); setMsg('저장되었습니다.'); setTimeout(() => setMsg(''), 3000); },
  });
  ...
  const changePassword = useMutation({
    mutationFn: async (body: { currentPassword: string; newPassword: string }) => {
      ...
      if (!r.ok) { const err = await r.json(); throw new Error(err.error?.message); } return r.json();
    },
    ...
    <div className="max-w-2xl space-y-6">
      ...
      <div className="bg-white rounded-lg border border-slate-200 p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          {!editMode && <Button size="sm" variant="secondary" onClick={() => { setEditMode(true); setName(user?.name || ''); }}>수정</Button>}
        </div>
        ...
        {editMode ? (
          <div className="space-y-3">
            ...
            <div className="flex gap-2">
              <Button size="sm" onClick={() => updateProfile.mutate({ name })}>저장</Button>
          ...
          <div className="space-y-3 text-sm">
            <div className="flex items-center gap-3">
              <div className="w-14 h-14 rounded-full bg-blue-100 flex items-center justify-center"><User className="h-6 w-6 text-blue-600" /></div>
              <div>
      ...
      <div className="bg-white rounded-lg border border-slate-200 p-6">
        ...
        {pwMsg.text && <div className={`p-2 rounded text-sm mb-3 ${pwMsg.type === 'success' ? 'bg-emerald-50 border-emerald-200 text-emerald-700' : 'bg-red-50 border-red-200 text-red-700'} border`}>{pwMsg.text}</div>}
        {pwMode ? (
          <div className="space-y-3">
            <div><label className="block text-sm font-medium text-slate-700 mb-1">현재 비밀번호</label><input type="password" value={currentPw} onChange={e => setCurrentPw(e.target.value)} className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm" /></div>
            <div><label className="block text-sm font-medium text-slate-700 mb-1">새 비밀번호 (8자 이상)</label><input type="password" value={newPw} onChange={e => setNewPw(e.target.value)} className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm" /></div>
            <div><label className="block text-sm font-medium text-slate-700 mb-1">새 비밀번호 확인</label><input type="password" value={confirmPw} onChange={e => setConfirmPw(e.target.value)} className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm" />{confirmPw && newPw !== confirmPw && <p className="text-xs text-red-500 mt-1">불일치</p>}</div>
            <div className="flex gap-2">
              <Button size="sm" onClick={() => changePassword.mutate({ currentPassword: currentPw, newPassword: newPw })} disabled={!currentPw || newPw.length < 8 || newPw !== confirmPw}>변경</Button>
